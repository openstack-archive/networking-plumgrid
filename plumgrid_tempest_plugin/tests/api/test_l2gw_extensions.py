...
class L2GatewayExtensionTestJSON(base.BaseAdminNetworkTest):
    ...
    def test_create_l2gw_conn_with_segid_when_l2gw_created_with_segid(self):
        # Create an L2Gateway
        gw_name = data_utils.rand_name('l2gw')
        vtep_ifc = '1'
        vtep_ip = '3.3.3.3/32'
    ...
    def test_create_update_l2gw_with_multiple_devices(self):
        ...
        seg_id_updated = random.sample(range(2, 4095), 1)
        device_updated = [{"device_name": device_name_list[0],
                           "device_ip": "192.168.10.100",
                           "interfaces":[{"segmentation_id": seg_id_updated,
                                          "name": "intNameNew"}]}]
        interface_name_list = [
