...
function usage {
  echo "Usage: $0 [OPTION]..."
  echo "Run PLUMgrid neutron's test suite(s)"
  echo ""
...
function run_tests {
  ...
  if [ $debug -eq 1 ]; then
    if [ "$testopts" = "" ] && [ "$testargs" = "" ]; then
      ...
      # provided.
      testargs="discover ./networking_plumgrid/neutron/tests"
    fi
  ...
  OS_TEST_PATH=`echo $testargs|grep -o 'networking_plumgrid\.neutron\.tests[^[:space:]:]*\+'|tr . /`
  ...
  if [ $coverage -eq 1 ]; then
    ...
    ${wrapper} coverage html --include='networking_plumgrid/neutron/*' --omit='networking_plumgrid/neutron/openstack/common/*' -d covhtml -i
  fi
