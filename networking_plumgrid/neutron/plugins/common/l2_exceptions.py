...
class L2GatewayServiceNotFound(exceptions.NotFound):
    message = _("L2 Gateway Service %(gateway_id)s could not be found")


class L2GatewayServiceInUse(exceptions.InUse):
    message = _("L2 Gateway Service '%(gateway_id)s' still has active mappings"
...
class L2gatewaySegmentationIDNotFound(exceptions.NotFound):
    message = _("The segmentation id is not found on gateway "
                "'%(gateway_id)s'")
...
class L2GatewaySegmentationRequired(exceptions.NeutronException):
    message = _("L2 gateway segmentation id is required and must be "
                "consistent for all the interfaces")
