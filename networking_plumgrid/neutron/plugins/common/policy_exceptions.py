...
class PolicyServiceChainInUse(exceptions.InUse):
    message = _("Policy Service Chain %(id)s %(reason)s with policy "
                "service(s): %(ps)s")
...
class PolicyServiceAlreadyInUse(exceptions.InUse):
    message = _("Policy Service '%(ps)s' already in use by a policy service "
                "chain. Cannot be associated with policy service chain "
                "'%(psc)s'.")
...
class InvalidFormatActionTarget(exceptions.InvalidInput):
    message = _("Invalid input format for action-target. Use the following "
                "format '<tenant_id:service-name/uuid OR "
                "tenant_id:service-chain-id/name>'")
...
class UnsupportedAction(exceptions.InvalidInput):
    message = _("Action (%(action)s) not supported for endpoint policy. "
                "Supported action(s) are ['copy', 'allow', 'redirect'].")
