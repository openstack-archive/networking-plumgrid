    ...
    if data is not None:
        ...
        if len(valid_ip_port) != 3:
            ...
            try:
                ...
                if (int(valid_ip_port[1]) >= 0 and
                    int(valid_ip_port[1]) <= 65535):
                    if not (int(valid_ip_port[2]) >= 0 and
                            int(valid_ip_port[2]) <= 16):
                        raise InvalidIPPortFormat("Error: Invalid mask. "
                                             "Please use mask in range (0-16)")
                else:
