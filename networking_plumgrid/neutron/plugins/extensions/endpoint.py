...
class InvalidIPPortFormat(nexceptions.InvalidInput):
    message = _("Invalid format for ip_port. Please specify "
                "correct ip_port: Format ('IP_PORT_MASK')")
    def __init__(self, error):
            ...
            try:
                ...
                if (int(valid_ip_port[1]) >=0 and int(valid_ip_port[1]) <=65535):
                    if not(int(valid_ip_port[2]) >= 0 and int(valid_ip_port[2]) <=16):
                        raise InvalidIPPortFormat(": Invalid mask. Please use "
                                "mask in range (0-16)")
                else:
                    raise InvalidIPPortFormat(": Invalid port. Please "
...
def _validate_label(data, valid_values=None):
    if data is not None:
        raise Exception("Association type 'label' is not supported")
...
validators.validators['type:validate_label'] = _validate_label
