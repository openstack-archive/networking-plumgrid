...
PG_POLICY_SVC_CHAIN = 'policy_service_chain'
PG_POLICY_SVC_CHAINS = 'policy-service-chains'
...
def _validate_service_list(data, valid_values=None):
    if not isinstance(data, list):
        raise InvalidPortFormat("Must be a list of policy services.")

    for service in data:
        if 'id' not in service:
            raise InvalidPortFormat("id field is required")

validators.validators['type:validate_service_list'] = _validate_service_list
...
RESOURCE_ATTRIBUTE_MAP = {
    'policy_service_chain': {
        ...
        'services': {
            ...
            'default': [],
            'validate': {
                'type:validate_service_list': None
            }
        },
        'add_services': {
            'allow_post': False,
...
class Policyservicechain(extensions.ExtensionDescriptor):

    @classmethod
    def get_name(cls):
        return "Policy Service Chain"
    ...
    def get_description(cls):
        return "This API will be used to create policy" \
               " service chains from Neutron and map it in PLUMgrid."

    @classmethod
    def get_namespace(cls):
        return "http://docs.openstack.org/ext/policy_service_chain" \
               "/api/v2.0"
    ...
    def get_resources(cls):
        ...
        resource_name = PG_POLICY_SVC_CHAIN
        collection_name = PG_POLICY_SVC_CHAINS
