...
class PolicyTag(model_base.BASEV2, models_v2.HasId,
    ...
    floatingip_id = sa.Column(sa.String(36),
                            ...
                            ondelete="CASCADE"),
                            nullable=True)
    floatingip_address = sa.Column(sa.String(attributes.LONG_DESCRIPTION_MAX_LEN))
    tag_id = sa.Column(sa.String(attributes.LONG_DESCRIPTION_MAX_LEN))
    tenant_id = sa.Column(sa.String(attributes.LONG_DESCRIPTION_MAX_LEN))

    fp_binding = orm.relationship(FloatingIP,
...
class PolicyTagMixin(common_db_mixin.CommonDbMixin):
    def create_policy_tag(self, context, policy_tag):
        ...
        with context.session.begin(subtransactions=True):
            ptag_db = PolicyTag(tenant_id=ptag["tenant_id"],
                                ...
                                floatingip_id=ptag["floating_ip"],
    ...
    def _make_ptag_dict(self, ptag, fields=None):
        ptag_dict = {"id": ptag.id,
                    ...
                    "tag_type": ptag.tag_type,
                    "tag_id": ptag.tag_id,
                    "floating_ip": ptag.floatingip_id,
