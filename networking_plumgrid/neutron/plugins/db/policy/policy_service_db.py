...
class PolicyServiceMixin(common_db_mixin.CommonDbMixin):
    def create_policy_service(self, context, policy_service):
        ...
        # TODO(muawiakhan): FIXME Can a port be classified into two policy services
        self._check_port_ps_association(context, ps["ingress_ports"], constants.DIRECTION_INGRESS)
        self._check_port_ps_association(context, ps["egress_ports"], constants.DIRECTION_EGRESS)
    ...
    def _check_port_ps_association(self, context, ports, direction):
        for port in ports:
            query = self._model_query(context, PolicyServicePortBinding)
            port_id = port["id"]
            try:
                port_in_use = query.filter_by(port_id=port_id, direction=direction).one()
                if port_in_use:
    ...
    def _make_ps_dict(self, ps, ps_ingress_ports=None, ps_egress_ports=None, fields=None):
        ingress_ports = []
        egress_ports = []
        ...
        #for ingress_port in ps_ingress_ports:
        #    ingress_ports.append({"id": ingress_port})
        #for egress_port in ps_ingress_ports:
        #    egress_ports.append({"id": egress_port})
        ps_dict = {"id": ps.id,
                    "name": ps.name,
