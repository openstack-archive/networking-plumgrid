...
from networking_plumgrid.neutron.plugins.db.policy.endpoint_db import \
    Endpoint
...
class PolicyServiceMixin(common_db_mixin.CommonDbMixin):
    def create_policy_service(self, context, policy_service):
        ...
        Args:
             policy_service:
                   ...
                   ingress_ports : List of ingress ports to be classified into policy service
                   egress_ports: List of egress ports to be classified into policy service
                   bidirectional_ports: List of bidirectional ports to be classified into policy service
        """
        ...
        if "ingress_ports" in ps and ps["ingress_ports"]:
            ...
            self._check_port_endpoint_association(context, ps["ingress_ports"])
        if "egress_ports" in ps and ps["egress_ports"]:
            self._check_port_ps_association(context, ps["egress_ports"])
            self._check_port_endpoint_association(context, ps["egress_ports"])
        if "bidirectional_ports" in ps and ps["bidirectional_ports"]:
            self._check_port_ps_association(context, ps["bidirectional_ports"])
            self._check_port_endpoint_association(context, ps["bidirectional_ports"])
        LOG.info(_LI("Creating policy service %(name)s."), {'name': ps["name"]})
        ...
        with context.session.begin(subtransactions=True):
            for ingress_port in ps["ingress_ports"]:
                ps_ingress_port_db = PolicyServicePortBinding(port_id=ingress_port["id"],
            ...
            for egress_port in ps["egress_ports"]:
                ps_egress_port_db = PolicyServicePortBinding(port_id=egress_port["id"],
                                         policy_service_id=ps_db.id,
                                         direction=constants.DIRECTION_EGRESS)
                context.session.add(ps_egress_port_db)
            for bidirect_port in ps["bidirectional_ports"]:
                ps_bidirect_port_db = PolicyServicePortBinding(port_id=bidirect_port["id"],
                                         policy_service_id=ps_db.id,
    ...
    def update_policy_service(self, context, ps_id, policy_service):
        ...
        Args:
             ...
             policy_service:
                   ...
                   add_ingress_ports: List of ingress ports to be added to policy service
                   remove_ingress_ports: List of ingress ports to be removed from policy service
                   add_egress_ports: List of egress ports to be added to policy service
                   remove_egress_ports: List of egress ports to be removed from policy service
        """
        ps = policy_service["policy_service"]
        if not ps:
        ...
        with context.session.begin(subtransactions=True):
            ...
            if 'add_ingress_ports' in ps:
                self._check_port_ps_association(context, ps["add_ingress_ports"])
                self._check_port_endpoint_association(context, ps["add_ingress_ports"])
            ...
            if 'add_egress_ports' in ps:
                self._check_port_ps_association(context, ps["add_egress_ports"])
                self._check_port_endpoint_association(context, ps["add_egress_ports"])
            ...
            if 'add_bidirectional_ports' in ps:
                self._check_port_ps_association(context, ps["add_bidirectional_ports"])
                self._check_port_endpoint_association(context, ps["add_bidirectional_ports"])
    ...
    def _check_port_endpoint_association(self, context, ports):
        for port in ports:
            ...
            try:
                port_in_use = query.filter_by(port_id=port_id).one()
                if port_in_use:
                    raise p_excep.PortAlreadyInUseEndpoint(port=port_id,
                              ep=port_in_use.id)
            except exc.NoResultFound:
                pass
