...
class EndpointBinding(model_base.BASEV2):
    ...
    __tablename__ = "pg_endpoint_binding"

    ep_grp_name = sa.Column(sa.String(attributes.NAME_MAX_LEN))
    endpoint_id = sa.Column(sa.String(36),
                      sa.ForeignKey("pg_endpoints.id",
                                    ondelete='CASCADE'),
                      primary_key=True)

    ep_grp_id = sa.Column(sa.String(36),
    ...
    ep = orm.relationship(Endpoint,
              backref=orm.backref("ep_binding",
                                  lazy="joined",
                                  cascade="all,delete"),
    primaryjoin="Endpoint.id==EndpointBinding.endpoint_id")

    epg = orm.relationship(EndpointGroup,
              backref=orm.backref("epg_binding",
                                  lazy="joined",
                                  cascade="all,delete"),
    primaryjoin="EndpointGroup.id==EndpointBinding.ep_grp_id")
...
class EndpointMixin(common_db_mixin.CommonDbMixin):
    def create_endpoint(self, context, endpoint):
        ...
        # Check availability of IP:port/mask
        (ip_address, port_mask) = self._get_ip_port_mask(ep)
        ...
        with context.session.begin(subtransactions=True):
            ...
            for epg in ep["ep_groups"]:
                ep_grp_db = EndpointBinding(endpoint_id=ep_db.id,
                                            ep_grp_id=epg["id"],
                                            ep_grp_name=epg["name"],
                                            ep=ep_db)
    ...
    def _check_ep_in_use(self, context, ep_db):
        ...
        for epg in ep_db.ep_binding:
            epg_id = epg.ep_grp_id
            query = self._model_query(context, EndpointBinding)
            try:
                epg_in_use = query.filter_by(ep_grp_id=epg_id).one()
                if epg_in_use:
    ...
    def _get_ip_port_mask(self, ep):
        ip_address = None
        port_mask = None
        if "ip_port_mask" in ep and ep["ip_port_mask"]:
            ip_address = ep["ip_port_mask"].split(':')[0]
            port_mask = ep["ip_port_mask"].split(':')[1]
        return(ip_address, port_mask)

    def _make_ep_dict(self, ep, fields=None):
        epg_list = []
        for epg in ep.ep_binding:
            epg_list.append({"id": epg.ep_grp_id,
                             "ep_grp_name": epg.ep_grp_name})
        ep_dict = {"id": ep.id,
                   "name": ep.name,
                   "ep_groups": epg_list,
                   "label": ep.label,
