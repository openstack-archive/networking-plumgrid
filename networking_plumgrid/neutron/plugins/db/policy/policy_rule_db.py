...
from networking_plumgrid.neutron.plugins.db.policy.policy_tag_db \
    import PolicyTag
from networking_plumgrid.neutron.plugins.db.policy.policy_service_chain_db \
    import PolicyServiceChain
...
class PolicyRule(model_base.BASEV2, models_v2.HasId,
    ...
    send_to_service_chain = orm.relationship(PolicyServiceChain,
                              backref=orm.backref('service_chain_binding',
                              cascade='all,delete'),
                    primaryjoin="PolicyServiceChain.id==PolicyRule.action_target_service_chain")
...
class PolicyRuleMixin(common_db_mixin.CommonDbMixin):
    def create_policy_rule(self, context, policy_rule):
        ...
        with context.session.begin(subtransactions=True):
            pr_db = PolicyRule(tenant_id=pr["tenant_id"],
                               ...
                               action_target_service_chain=pr["action_target_service_chain"],
    ...
    def _validate_src_grp_policy_rule_config(self, context, pr):
        ...
        if pr["src_grp"] is not None:
            ...
            if self._check_endpoint_group_for_policy_rule(context, pr["src_grp"]):
                pr["src_security_group"] = is_security_group
                return
            if self._check_security_group_for_policy_rule(context, pr["src_grp"]):
    ...
    def _validate_dst_grp_policy_rule_config(self, context, pr):
        ...
        if pr["dst_grp"] is not None:
            is_security_group = False
            if self._check_endpoint_group_for_policy_rule(context, pr["dst_grp"]):
                pr["dst_security_group"] = is_security_group
                return
            if self._check_security_group_for_policy_rule(context, pr["dst_grp"]):
                is_security_group = True
                pr["dst_security_group"] = is_security_group
                return
    ...
    def _make_pr_dict(self, pr, fields=None):
        ...
        action_target = ((pr.action_target_tenant_id if
                          pr.action_target_tenant_id else "") +
                          (pr.action_target_service if
                           pr.action_target_service else "") +
                          (pr.action_target_service_chain if
                           pr.action_target_service_chain else ""))
    ...
    def _validate_action_target(self, context, pr):
        ...
        if pr["action_target"] is None:
            ...
            pr["action_target_service_chain"] = None
            pr["action_target_tenant_id"] = None
        else:
            action_target_len = len(pr["action_target"].split(":"))
            ...
            if tenant_id is None:
                try:
                    ...
                    try:
                        ...
                        psc_db = query.filter_by(id=action_target_id).one()
                        if psc_db["id"] == action_target_id:
                            pr["action_target_service"] = None
                            pr["action_target_service_chain"] = pr["action_target"]
                            pr["action_target_tenant_id"] = None
                    except exc.NoResultFound:
                ...
                try:
                    ...
                    try:
                        query = context.session.query(PolicyServiceChain)
                        psc_db = query.filter_by(id=action_target_id,
                                                 tenant_id=tenant_id).one()
                        if psc_db["id"] == action_target_id:
                            pr["action_target_service"] = None
                            pr["action_target_service_chain"] = action_target_id
                            pr["action_target_tenant_id"] = tenant_id
                    except exc.NoResultFound:
                        raise p_excep.NoActionTargetFound(at=action_target_id)
            ...
            try:
              ...
              pr["tag"] = pt_db["id"]
                ...
                try:
                    ...
                    if("policy_tag_id" not in epg_db or epg_db["policy_tag_id"] is None):
