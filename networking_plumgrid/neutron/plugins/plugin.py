...
class NeutronPluginPLUMgridV2(agents_db.AgentDbMixin,
    ...
    def create_endpoint_group(self, context, endpoint_group):
        LOG.info_("networking_plumgrid: Endpoint group create method "
                 "called")
        vm_mac_list = []
    ...
    def delete_endpoint_group(self, context, id):
        LOG.info_("networking_plumgrid: Endpoint group delete method "
                 "called")
    ...
    def update_endpoint_group(self, context, id, endpoint_group):
        LOG.info_("networking_plumgrid: Endpoint group update method "
                 "called")
        add_vm_mac_list = []
        remove_vm_mac_list = []
    ...
    def get_endpoint_group(self, context, id, fields=None):
        LOG.info_("networking_plumgrid: Get endpoint group "
                  "called")
        return super(NeutronPluginPLUMgridV2,
    ...
    def get_endpoint_groups(self, context, filters=None,
                           fields=None, sorts=None, limit=None,
                           marker=None, page_reverse=False):
        LOG.info_("networking_plumgrid: List endpoint groups"
                  "called")
        return super(NeutronPluginPLUMgridV2,
    ...
    def create_endpoint_policy(self, context, endpoint_policy):
        LOG.info_("networking_plumgrid: Create endpoint policy method called")
    ...
    def delete_endpoint_policy(self, context, id):
        LOG.info_("networking_plumgrid: Delete Endpoint policy "
    ...
    def _process_epp(self, context, epp_db):
        if not uuidutils.is_uuid_like(epp_db['src_grp']):
            ...
            if len(src_grp_list) == 1:
                ...
                raise plum_excep.PLUMgridException(err_msg=err_message)
        if not uuidutils.is_uuid_like(epp_db['dst_grp']):
            ...
            if len(dst_grp_list) == 1:
                ...
                raise plum_excep.PLUMgridException(err_msg=err_message)
        if not uuidutils.is_uuid_like(epp_db['service_endpoint_group']):
