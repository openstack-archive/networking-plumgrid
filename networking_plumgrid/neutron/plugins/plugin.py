...
class NeutronPluginPLUMgridV2(agents_db.AgentDbMixin,
        ...
        if not network_type_set:
            ...
            if not physical_network_set:
                ...
                if not uuidutils.is_uuid_like(physical_network):
                    physical_network = self._process_phy_net(context, physical_network)
            ...
            if not physical_network_set:
                msg = _("provider:physical_network required")
                raise n_exec.InvalidInput(error_message=msg)
            else:
                if not uuidutils.is_uuid_like(physical_network):
                    physical_network = self._process_phy_net(context, physical_network)
    ...
    def create_physical_attachment_point(self, context,
        ...
        physical_attachment_point["physical_attachment_point"] = self._process_pap(context, pap_obj)
    ...
    def _process_phy_net(self, context, pap_name):
        pap_id_list = self.get_physical_attachment_points(context, filters={'name': [pap_name]}, fields=["id"])
        if len(pap_id_list) == 1:
            return pap_id_list[0]["id"]
        elif len(pap_id_list) == 0:
            err_message = _("No physical_attachment_point matches found for name"
                            "'%s'" % pap_name)
            raise plum_excep.PLUMgridException(err_msg=err_message)
        else:
            err_message = _("Multiple physical_attachment_point matches found for name"
    ...
    def _process_pap(self, context, pap_db):
        if not uuidutils.is_uuid_like(pap_db['transit_domain_id']):
            td_id_list  = self.get_transit_domains(context, filters={'name': [pap_db['transit_domain_id']]}, fields=["id"])
            if len(td_id_list) == 1:
                ...
                err_message = _("Multiple physical_attachment_point matches found for name"
                                "'%s', use an ID to be more specific." % pap_db['transit_domain_id'])
                raise plum_excep.PLUMgridException(err_msg=err_message)
        return pap_db
