...
class NeutronPluginPLUMgridV2(db_base_plugin_v2.NeutronDbPluginV2,
    ...
    def create_subnet(self, context, subnet):
        ...
        with context.session.begin(subtransactions=True):
            ...
            sb_db = super(NeutronPluginPLUMgridV2, self).create_subnet(context,
                                                                        subnet)
        return self._create_subnet_pg(context, subnet, net_db, sub_db, ipnet,
                                      tenant_id)

    @pgl
    def _create_subnet_pg(self, context, subnet, net_db, sub_db, ipnet,
                          tenant_id):
        s = subnet['subnet']
        try:
    ...
    def _delete_subnet_pg(self, context, subnet_id, net_db, net_id, sub_db,
        ...
        with context.session.begin(subtransactions=True):
            ...
            super(NeutronPluginPLUMgridV2, self).delete_subnet(context,
                                                               subnet_id)
