...
class NeutronPluginPLUMgridV2(agents_db.AgentDbMixin,
    ...
    def create_endpoint(self, context, endpoint):
        ...
        pg_helper._is_security_group(context, self, endpoint["endpoint"], "ep_groups")
        with context.session.begin(subtransactions=True):
    ...
    def update_endpoint(self, context, id, endpoint):
        ...
        with context.session.begin(subtransactions=True):
            ...
            try:
                ...
                self._plumlib.update_endpoint(ep["tenant_id"], id, ep, port_mac=port_mac_address)
            except Exception as err_message:
                raise plum_excep.PLUMgridException(err_msg=err_message)
        return ep
    ...
    def update_policy_service(self, context, id, policy_service):
        ...
        with context.session.begin(subtransactions=True):
            ...
            ps['add_bidirectional_ports'] = ps_db.get("add_bidirectional_ports", [])
            ps['remove_ingress_ports'] = ps_db.get("remove_ingress_ports", [])
            ps['remove_egress_ports'] = ps_db.get("remove_egress_ports", [])
            ps['remove_bidirectional_ports'] = ps_db.get("remove_bidirectional_ports", [])
            try:
    ...
    def _process_ep_update(self, context, ep_db):
        if "add_endpoint_groups" in ep_db and ep_db["add_endpoint_groups"]:
            self._process_endpoint_groups(context, ep_db, "add_endpoint_groups")
        if "remove_endpoint_groups" in ep_db and ep_db["remove_endpoint_groups"]:
            self._process_endpoint_groups(context, ep_db, "remove_endpoint_groups")
        return ep_db
