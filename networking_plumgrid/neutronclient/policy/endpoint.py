...
def args2body(self, parsed_args):
    try:
        ...
        if ((parsed_args.port_id or parsed_args.ip_mask or
             parsed_args.label or parsed_args.ip_port_mask) and not
        ...
        if (parsed_args.ep_groups and (not parsed_args.port_id and not
            parsed_args.ip_mask and not parsed_args.label and not
            parsed_args.ip_port_mask)):
            raise Exception("Please specify an association criteria.")
        if ((parsed_args.ip_port_mask and parsed_args.label) or
            (parsed_args.ip_port_mask and parsed_args.ip_mask) or
            (parsed_args.ip_port_mask and parsed_args.port_id) or
            (parsed_args.label and parsed_args.ip_mask) or
            (parsed_args.label and parsed_args.port_id) or 
        ...
        if parsed_args.label:
            raise Exception("Association type 'label' is currently unsupported.")
        if parsed_args.name:
...
class EndpointCreate(extension.ClientExtensionCreate,
    ...
    def add_known_arguments(self, parser):
        ...
        parser.add_argument('--ip-port', dest='ip_port_mask',
    ...
    def args2body(self, parsed_args):
        try:
            ...
            if (parsed_args.add_endpoint_groups and
                ...
                if len(set([tuple(d.items()) for d in add_endpoint_group_dict]).
