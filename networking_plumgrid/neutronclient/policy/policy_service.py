...
def add_known_arguments(self, parser):
    parser.add_argument(
        ...
        help=_('Ingress ports to be classified into policy service.'
    ...
    parser.add_argument(
        ...
        type=utils.str2dict_type(optional_keys=['id', 'name']),
        help=_('Egress ports to be classified into policy service.'
...
class PolicyServiceCreate(extension.ClientExtensionCreate,
    ...
    def add_known_arguments(self, parser):
        parser.add_argument(
            'name', metavar='<POLICY_SERVICE-NAME>',
...
class PolicyServiceUpdate(extension.ClientExtensionUpdate,
    ...
    def add_known_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=utils.str2dict_type(optional_keys=['id', 'name']),
            help=_('Ports/Servers to be added to policy service. '
                   'id=<ingress_port_uuid>,name=<ingress_port_name>'
                   '(--add-ingress-ports option can be repeated)'))
        parser.add_argument(
            '--add-egress-port',
            metavar='id=ID,name=NAME',
            ...
            help=_('Ports/Servers to be added to policy service. '
                   'id=<egress_port_uuid>,name=<egress_port_name>'
                   '(--add-egress-ports option can be repeated)'))
        parser.add_argument(
            '--remove-ingress-port',
            metavar='id=ID,name=NAME',
            action='append', dest='remove_ingress_ports',
            type=utils.str2dict_type(optional_keys=['id', 'name']),
            help=_('Ports/Servers to be removed from policy service. '
                   'id=<ingress_port_uuid>,name=<ingress_port_name>'
                   '(--remove-ingress_ports option can be repeated)'))
        parser.add_argument(
            '--remove-egress-port',
            metavar='id=ID,name=NAME',
            ...
            help=_('Ports/Servers to be removed from policy service. '
                   'id=<egress_port_uuid>,name=<egress_port_name>'
                   '(--remove-egress_ports option can be repeated)'))
        parser.add_argument('--description', dest='description',
                            help=_('Description of'
                            ' policy service.'))
    ...
    def args2body(self, parsed_args):
        try:
            ...
            if parsed_args.description:
                body['policy_service']['description'] = parsed_args.description
